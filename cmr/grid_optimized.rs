...
impl Grid {
    ...
    fn set(&mut self, (x, y): (uint, uint), val: u8) {
        let loc = self.at(x, y);
        unsafe { self.cells.unsafe_set(loc, val); }
    }

    fn get(&self, (x, y): (uint, uint)) -> u8 {
        let s: &[u8] = self.cells;
        unsafe { *s.unsafe_ref(self.at(x, y)) }
    }
    ...
    fn solve_inner(&mut self, (x,  y): (uint, uint), value: u8) -> bool {
        ...
        // todo: unroll neighbors loop, don't use vector/iterators (makes an
        // allocation)
...
fn main() {
    //let mut g: Grid = from_str("9 53 52 51 50 5 4 3 2 1 10 11 14 15 28 29 32 33 34 35 36 37 72 73 76 77 78 79 66 65 64 63 54").unwrap();
    let mut g: Grid = from_str("8 1 8 9 16 17 48 49 50 51 64 63 62 61 60 59 58 41 40 35 34 29 28 27 26 25 4 3 2").unwrap();
    g.solve();
    g.display();
